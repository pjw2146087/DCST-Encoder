...
class _DatasetFormat:
    ...
    @staticmethod
    def create_fetcher(_mode, filenames):
        if _mode == _DatasetFormat.Trace:
            return _utils.read._TraceDatasetReader(filenames)
        else:
            return _utils.read._PointDatasetReader(filenames)
        
class _DatasetMode:
    _all_features_=['43_features']
    _all_adj_=['adj_simple']
    
    @staticmethod
    def create_processer(_mode, filenames,path_save):
        if _mode in _DatasetMode._all_features_:
            return _utils.preprocess._features_Processer(filenames,path_save,_mode)
        elif _mode in _DatasetMode._all_adj_:
            return _utils.preprocess._adj_Processer(filenames,path_save,_mode)

class _DatasetSplit:
    _write_ = ['Identically_Distributed']
    _move_ = ['Non-Identically_Distributed']
    
    @staticmethod
    def create_writer(data ,path, mode, data_type,filenames):
        return _utils.write._TraceDatasetWriter(data ,path ,mode ,data_type,filenames)
    def create_mover(filenames ,path,mode, data_type):
        return _utils.move._TraceDatasetmover(filenames ,path, mode, data_type)
