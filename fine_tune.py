...
model = DC_encoder(
    ...
    attn_drop_rate=0.6, 
    drop_path_rate=0.4,
    pretrained_path='./weights/pre_model.pt', #如果是None就表示不使用预训练
).to(device)
...
for pass_num in range(total_epochs):
    ...
    for batch_id, (points, labels, adjs, trace_id) in enumerate(train_loader()):
        ...
        loss, acc = model.train_step(data, labels,optimizer, None)
    ...
    with torch.no_grad():
        ...
        for batch_id, (points, labels, adjs, trace_id) in enumerate(valid_loader()):
            ...
            edge_index = torch.stack([rows, cols]).to(device)
            data = Data(x=points, edge_index=edge_index, y=labels)
            loss, acc = model.valid_step(data, labels, None)
            trajectory_length = points.shape[0]
        ...
        if final_test == True:
            final_class_result = class_result
            final_test = False
